...
public class userCPuUtilization extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String query = "SELECT sensor_id FROM system272.sensor_master where user_id =" + user_id;
		...
		if (sensor_ids != null) {
			try {
				...
				for (int i = 0; i < sensor_ids.size(); i++) {
					String sensorname = "SELECT sensor_tag_value FROM system272.sensor_master where sensor_id='"
							+ sensor_ids.get(i) + "'";
	...
	private ArrayList<ArrayList<Double>> getUtilizationData(ArrayList<String> sensor_ids, ArrayList<String> dates)
		...
		for (int i = 0; i < sensor_ids.size(); i++) {
			...
			String query = "SELECT date, AVG(cpu_data) as avgutil FROM system272.utilization where sensor_id='"
					+ sensor_ids.get(i) + "' group by CAST(date AS date) order by date DESC limit 10";
