...
public class InvokeInstance {
	...
	public void createSensor(String user_id, String sensor_name, String lat, String lon) throws InterruptedException {
		...
		AmazonEC2 amazonEC2Client = new AmazonEC2Client(
				new BasicAWSCredentials("AKIAILV4QDH3G3CBPOVQ", "6DiMKw9OQq9uKiEH/RSZxx+3InaaEkp2cxlCTsSS"));
		// AKIAI3VYQX3UB3W6KOMA
		// Z0HdxoiVABR/B0J0zw06heg/G8wdNYsze3rO/yHw

		// new AmazonEC2Client(new BasicAWSCredentials("AKIAI32NAJBWBXFWKXPQ",
		// "fbQTu9ERdPKdkKFycmnDIgMpQ2ziphaiFIHjXUw6"));
		...
		runInstancesRequest.withImageId("ami-a9d276c9") // ami-9f5efbff
														// //ami-a9d276c9)
				.withInstanceType("t2.micro").withMinCount(1).withMaxCount(1).withKeyName("cmpe272key") // class

				// .withKeyName("Hello_281")
				.withSecurityGroups("launch-wizard-1"); // DB-Security-Group
		 ...
		 * 
		 * com.amazonaws.services.ec2.model.Tag tags = new
		 * com.amazonaws.services.ec2.model.Tag("Name",sensor_name); try{
		 * createTagsRequest.withResources(runInstancesResult.getReservation().
		 * getInstances().get(0).getInstanceId()).withTags(tags );
		 * amazonEC2Client.createTags(createTagsRequest); } catch(Exception e) {
		 * e.printStackTrace(); }
		 */
		...
		String groupId = ires.getReservations().get(0).getInstances().get(0).getSecurityGroups().get(0).getGroupId()
				.toString();

		String groupname = ires.getReservations().get(0).getInstances().get(0).getSecurityGroups().get(0).getGroupName()
				.toString();
		...
		// String tag_key =
		// ires.getReservations().get(0).getInstances().get(0).getTags().get(0).getKey();
		...
		System.out
				.println("sensor_2:" + sensor_id + "," + lat + "," + lon + "," + status_new + "," + "," + sensor_name);
		String name = "Name";
		String query = "INSERT INTO sensor_master (sensor_id,latitude,longitude,sensor_status,public_ip,private_ip,creation_date,sensor_tag_key,sensor_tag_value,user_id,hub_id)"
				...
				+ "VALUES ('" + sensor_id + "','" + lat + "','" + lon + "','" + status_new + "','" + publicIp + "','"
				+ privateIp + "','" + sq + "','" + name + "','" + sensor_name + "','" + user_id + "'," + hub + ")";

		DatabaseAccess db = new DatabaseAccess();
		db.insert(query);

		String qq1 = "SELECT no_of_server FROM system272.hub_master where hub_id=" + hub;
		int no_of_server = db.getNoodfSensor(qq1);
		no_of_server++;

		String qu = "UPDATE `system272`.`hub_master` SET `no_of_server`='" + no_of_server + "' WHERE `hub_id`=" + hub;
		db.insert(qu);
